...
But there is another rule which is even more unbreakable. **Any events that need to stay in a fixed order must go in the same topic**. This a the cite from [Should You Put Several Event Types in the Same Kafka Topic?](https://www.confluent.io/blog/put-several-event-types-kafka-topic/)  by Martin Kleppmann Highly recommend it if you need to get more feeling about what can justify having different kinds of messages in a topic. [Putting Several Event Types in the Same Topic – Revisited](https://www.confluent.io/blog/multiple-event-types-in-the-same-kafka-topic/) unfolds the topic and shows how to use schema registries to formalize "one of" kind of schemas.

Schema registries are not the topic for today. Our focus is on the notion of event flow. Let us imagine that we have Kafka as a backend for event sourcing. Messages describe different subsequent updates of the state.
...
1) Easy configuration of property name treated as a discriminator. I had two concerns regarding hard-coded `"type"`:
...
2) Encoding JSONs with snake_case.
...
<details>
<summary>KafkaProducer snippet</summary>
...
<details>
<summary>KafkaConsumer snippets</summary>
